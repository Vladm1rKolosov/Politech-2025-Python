...
# 1. Сравниваются размерности двух массивов. Если размерности отличаются, то форма с наименьшей размерностью дополняется единицами с левой стороны

# 2. Если формы двух массивов не совпадают в каком то измерении, то массив с формой, равной 1 в данном измерении, растягивается до соответсвия форме другого раздела

# 3. Если в каком-либо измерении размеры массивов различабтся и ни один не равен 1, то генерируется ошибка
a = np.ones([2, 3])
b = np.arange(3)
c = np.ones((1, 3))
...
# Сколько элементов имеет значение больше 6
